...
| 3 | **Le illustrazioni** ✋ | Manus (in futuro: script TS) | un'immagine per pagina |
...
- **`lib/engine.ts`** — motore deterministico (port TS di `seme/scripts/*.py`).
  La suite **pytest** in `seme/tests/` resta il riferimento di parità.
...
lib/                  types, engine, commands, cache, stages, store, ai/
